...
**Dashboard-Specific Transformation Requirements:**

Each dashboard requires specific data transformations beyond basic field mapping:
- **Dashboard 1 (Income vs Expenses)**: 52-week rolling average calculations for wages and expenses
- **Dashboard 2 & 3 (Budget Views)**: Budget variance calculations, YTD/MTD aggregations
- **Dashboard 4 (WIP by Team)**: WIP calculation: `(Time Value + Billable Costs) - Progress Invoices`
- **Dashboard 6 (Services Analysis)**: Service type classification and billability rate calculations
- **Dashboard 7 (AR Aging)**: Aging bucket calculations based on due dates
- **Dashboard 8 (Recoverability)**: Outstanding invoice aggregations by client

**Comprehensive Transformation Reference:**
For complete transformation logic, formulas, and Power Query equivalents for all dashboards:
- **Primary Source**: `docs/xero-api-endpoint-mapping.md` (Sections for each dashboard)
- **Expanded Source**: `docs/xero-api-endpoint-mapping-expanded.md` (n8n implementation patterns)

[Source: xero-api-endpoint-mapping.md]

---

**Basic Invoice Transformation Example:**
```javascript
// n8n Function node
const invoice = $json;
...
### XPM API Integration (Future Epic 3)

**Note**: This story (Epic 1) focuses on Xero Accounting API integration only. XPM (Workflow Max) API integration for time tracking and WIP data will be implemented in Epic 3.

**XPM Endpoints Required** (for future implementation):
- `GET /job.api/list` - Jobs/Client data
- `GET /time.api/list` - Time entries
- `GET /cost.api/list` - Billable costs
- `GET /invoice.api/list` - Progress invoices
- `GET /staff.api/list` - Staff/team data
- `GET /job.api/task` - Task types for service classification

**Dashboards Requiring XPM Data**:
- Dashboard 4: Work In Progress by Team
- Dashboard 6: Services Analysis (time by service type)
- Dashboard 8: Client Recoverability (combines Xero AR + XPM WIP)

[Source: xero-api-endpoint-mapping.md#dashboard-4-6-8]
