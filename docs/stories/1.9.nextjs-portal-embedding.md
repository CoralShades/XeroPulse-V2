...
- [ ] **Task 5: Implement Role-Based Dashboard Routing** (AC: 5)
  ...
  - [ ] Executive → income-expenses, Manager → income-expenses, Staff → (future)
...
- ✅ Next.js 15 project initialized with App Router, Tailwind CSS, shadcn/ui
- ✅ Supabase Auth frontend integration (login, signup, logout pages)
- ✅ Authentication middleware for protected routes
- ✅ Supabase Auth backend configuration with RLS policies
- ✅ XeroPulse branding applied (logo, colors, typography)
- ✅ Vercel deployment configured
- [Source: Stories 1.0, 1.8]

**What This Story (1.9) Adds:**
- 🔧 Dashboard layout components (header, sidebar, main content)
- 🔧 Metabase iframe embedding with JWT-signed URLs
...
app/
├── (auth)/
│   ├── login/page.tsx         ✅ Story 1.0
│   ├── signup/page.tsx        ✅ Story 1.0
│   └── reset-password/        ✅ Story 1.0
├── layout.tsx                 ✅ Story 1.0
└── page.tsx                   ✅ Story 1.0

components/
├── ui/                        ✅ Story 1.0 (shadcn/ui)
└── auth/                      ✅ Story 1.0
...
│   │   └── income-expenses/
│   │       └── page.tsx      🆕 Income vs Expenses embedded dashboard
│   └── layout.tsx            🆕 Dashboard layout wrapper
└── api/
    └── dashboard-access/
        └── route.ts          🆕 Access logging API
...
└── dashboard/                🆕 This story
    ├── MetabaseEmbed.tsx    🆕 Iframe component
    └── DashboardError.tsx   🆕 Error UI

lib/
└── metabase/                 🆕 This story
    └── generateEmbedUrl.ts  🆕 JWT signing utility
```
...
```env
NEXT_PUBLIC_SUPABASE_URL=https://[project-ref].supabase.co   ✅ Story 1.0
NEXT_PUBLIC_SUPABASE_ANON_KEY=[anon-key]                    ✅ Story 1.0
SUPABASE_SERVICE_ROLE_KEY=[service-role-key]                ✅ Story 1.0
NEXT_PUBLIC_APP_URL=https://xeropulse.vercel.app            ✅ Story 1.0
```

**This Story Adds:**
```env
METABASE_SITE_URL=https://metabase.xeropulse.com           🆕 From Story 1.6
METABASE_SECRET_KEY=[embedding-secret-from-story-1.6]      🆕 This story uses
```
...
4. **Role-Based Routing Test (AC: 5):**
   - Log in as executive user → verify redirect to income-expenses
   - Log in as manager user → verify redirect to income-expenses
   - Log in as staff user → verify redirect to income-expenses
   - (Future) Log in as user with no dashboard access → verify error page
